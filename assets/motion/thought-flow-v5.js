/* Independent design study. Same tapered/rounded 5-stop trail language as
 * emotion-ball/js/ball.js, but short upward arcs outside the head, not an orbit. */
(function(root){
  'use strict';
  ...
  function create(host){
    ...
    function play(options={}){
      ...
      started=performance.now();elapsed=0;
      if(options.reducedMotion){draw(1750);return;}
      running=true;draw(0);raf=requestAnimationFrame(tick);
    }
  ...
  root.ThoughtFlowPreview=Object.freeze({create,sample,durationMs:DURATION});
})(window);
