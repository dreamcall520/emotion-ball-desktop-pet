...
- 原生模拟验收助手的候选代码已接入当前额度标签控制器，并使用本地合成快照覆盖 80/120/180/260 四档尺寸、常驻开关、自动/5 小时/周额度、气泡避让、浅深外观截图以及已用 10%/80%/90%/100% 的 normal/strong/urgent 表现。主代理已在下述真实窗口 smoke 中运行这些断言，但因旧截图帧问题，该轮不能写成完整实机验收通过；修复版仍须重新运行。
...
- 主代理执行的真实窗口 smoke 证据目录为 `/tmp/emotion-ball-quota-final.OTL9MS`，进程退出码与功能 marker 均为 0，但人工回读暴露出截图缺口：`quota-label-stale-long.png` 与新鲜浅色截图的 SHA-256 均为 `ad6e471ebfab8397e064f03aafc4047d6ac910c64db9e945f1967a1b6d78e220`，虽然页面 DOM 已进入过期状态，实际捕获仍是旧合成帧，因此该轮截图证据判定失败，不能因 marker 成功写成整体验收通过。`b995feb` 候选门禁现会在捕获前等待页面连续两帧及短暂绘制稳定，捕获前后都回读过期 DOM，并要求过期 PNG 与新鲜浅色、深色 PNG 均不同；相同内容会直接失败。`b995feb` 修复后的真实窗口 smoke 与人工截图复核尚未由主代理重跑。
...
- `b995feb` 修复后的真实 `npm run smoke`、新鲜浅深与过期截图仍尚未执行；打包应用、DMG/分享 ZIP、对应新哈希、本机安装替换、搜索入口及 GitHub Release 也未在本候选阶段执行。后续必须产生新鲜证据，不复用上文历史证据代替。
