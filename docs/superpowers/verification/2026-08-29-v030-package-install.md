...
- 定向命令 `node --test desktop-pet/tests/package-script.test.js desktop-pet/tests/codex-native-helper.test.js desktop-pet/tests/smoke-verification.test.js` 为 31 项通过、0 失败；`npm test` 为 702 项通过、0 失败、0 跳过。
...
- 常驻标签候选布局已把固定可见的「剩余 N%」放在可省略详情之前；模拟验收会以 64%、78% 和超长恶意标签读取真实 DOM 宽度，确认只截详情，并在截图前后核对仍是当前 176×54 标签窗口。四档尺寸只调用应用真实尺寸入口并回读窗口与设置，不能直接改宽高伪造通过标记。
...
- 异常收尾会先重建本地合成连接，再临时开启以恢复额度周期、常驻与任务名称开关；随后恢复原总开关、尺寸、位置、互动设置及调试器。各清理步骤独立执行，原始失败与清理失败会一并保留；不会因此连接真实 Codex。
